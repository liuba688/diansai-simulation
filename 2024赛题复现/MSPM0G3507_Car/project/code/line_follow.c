...
/* Direct position error and direct error delta are used by design. */
                         ...
                         float *right_target_rpm)
{
    ...
    if(!sensor->line_valid)
    {
        ...
        if(follow->lost_ticks <= LINE_FOLLOW_LOST_FORWARD_TICKS)
        {
            follow->mode = LINE_FOLLOW_MODE_LOST_SEARCH;
            /*
             * A brief missing sample may be a gap or reflection. Continue
             * straight at low speed instead of immediately steering aside.
             */
            *left_target_rpm = LINE_FOLLOW_LOST_FORWARD_RPM;
            *right_target_rpm = LINE_FOLLOW_LOST_FORWARD_RPM;
        }
        else
        {
