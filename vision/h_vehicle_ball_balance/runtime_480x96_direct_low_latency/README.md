...
```text
WAIT_CENTER -> HOLD_CENTER <-> RECOVER_CENTER
```

不会自动运行 `O -> +5 cm -> -5 cm`。台架接线、首次安全验证、日志字段和调参顺序见：
...
```text
dist/maix-h_task4-center-hold-bench-v4.0.0.zip
```

以下内容是原第一问基线说明，仅作历史参考。正式第一问候选版仍保存在
`versions/timed_full_task_v3_1_4_cal435_r3/`，没有被本次修改覆盖。
