...
CONTROL_PROFILE_NAME = "timed_full_task_v3_1_4_cal435_r1"
...
class BallTaskController:
    ...
    def _update_state(self, position_mm, velocity_mm_s, valid, now):
        ...
        if self.state == "POS_SETTLE":
            ...
            if (
                ...
                return
            if (
                self.state == "POS_SETTLE"
                and phase_time_s >= POSITIVE_SETTLE_TIMEOUT_S
            ):
                self._enter("NEG_PUSH", now)
            return
    ...
    def _command_angle(self, position_mm, velocity_mm_s):
        ...
        if self.state in ("NEG_SETTLE", "COMPLETE"):
            return self._settle_angle(
                NEGATIVE_TARGET_MM,
                position_mm,
                velocity_mm_s,
