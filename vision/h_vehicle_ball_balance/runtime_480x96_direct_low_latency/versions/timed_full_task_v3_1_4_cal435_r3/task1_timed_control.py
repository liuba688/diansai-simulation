...
CONTROL_PROFILE_NAME = "timed_full_task_v3_1_4_cal435_r3"
...
POSITIVE_PUSH_MAX_TIME_S = 0.50
POSITIVE_PUSH_MIN_TIME_S = 0.20
POSITIVE_BRAKE_POSITION_MM = 5.0
POSITIVE_BRAKE_SPEED_MM_S = 25.0

POSITIVE_BRAKE_ANGLE_DEG = -2.20
POSITIVE_BRAKE_MAX_TIME_S = 0.40
POSITIVE_BRAKE_MIN_TIME_S = 0.20
POSITIVE_SETTLE_ENTRY_MM = 30.0
POSITIVE_SETTLE_ENTRY_SPEED_MM_S = 120.0

NEGATIVE_PUSH_ANGLE_DEG = -1.40
NEGATIVE_PUSH_MAX_TIME_S = 0.90
NEGATIVE_PUSH_MIN_TIME_S = 0.30
NEGATIVE_BRAKE_POSITION_MM = 25.0
NEGATIVE_BRAKE_SPEED_MM_S = -90.0

NEGATIVE_BRAKE_ANGLE_DEG = 1.00
...
POSITIVE_SETTLE_SPEED_MM_S = 40.0
POSITIVE_SETTLE_TIME_S = 0.12
POSITIVE_SETTLE_TIMEOUT_S = 0.90
FINAL_SETTLE_ERROR_MM = 8.0
FINAL_SETTLE_SPEED_MM_S = 12.0
FINAL_SETTLE_TIME_S = 0.12
...
class BallTaskController:
    ...
    def _update_state(self, position_mm, velocity_mm_s, valid, now):
        ...
        if self.state == "POS_BRAKE":
            ...
            if (
                ready_to_settle
                or phase_time_s >= POSITIVE_BRAKE_MAX_TIME_S
            ):
                self._enter("POS_SETTLE", now)
            return

        if self.state == "POS_SETTLE":
            # Reconstructed from trials 14-18: reversal occurred only after
            # entering the +/-10 mm band with |v| <= 40 mm/s.
            if (
                ...
                return
            # Never reverse merely because the positive settle timer expired.
            # The scoring rule requires the ball to actually enter +40..+60 mm.
            # The global five-second timeout remains the escape path.
            return
        ...
        if self.state == "NEG_BRAKE":
            # Reconstructed v3.1.4 behavior: the fixed positive brake handed
            # control to PD once the ball was nearly stopped, even when it had
            # not yet reached -42 mm. This is the source of the observed
            # "early stop, then second negative push" trajectory.
            ready_to_settle = (
                phase_time_s >= NEGATIVE_BRAKE_MIN_TIME_S
                and (
                    position_mm <= NEGATIVE_SETTLE_ENTRY_MM
                    or velocity_mm_s >= -40.0
                )
            )
            if (
                ready_to_settle
    ...
    def _settle_angle(self, target_mm, position_mm, velocity_mm_s):
        ...
        if (
            ...
            and abs(angle_deg) < SETTLE_STICTION_MIN_ANGLE_DEG
        ):
            angle_deg = (
                SETTLE_STICTION_MIN_ANGLE_DEG
                if error_mm > 0.0
                else -SETTLE_STICTION_MIN_ANGLE_DEG
            )
