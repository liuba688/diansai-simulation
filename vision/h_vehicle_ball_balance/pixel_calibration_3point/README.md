...
> 本目录只用于读取 `0/+50/-50 mm` 三点的 `raw_x`。`main.py` 固定设置
> `PIXEL_CALIBRATION_ONLY=True`，控制器测试已验证电机目标始终为0。不要把本目录
> 当作第一问正式运行程序。

下方完整轨迹说明来自其控制工程母版，仅用于解释日志字段；本标定工程实际固定停留在
`PIXEL_CALIBRATION`，不会进入这些运动状态：
...
|---|---:|---:|---|
| `POS_PUSH` | `+2.20°` | `0.85 s` | `x>=+10 mm` 或 `v>=+35 mm/s` |
| `POS_BRAKE` | `-2.20°` | `0.28 s` | `x>=+42 mm` 且 `|v|<=30 mm/s` |
| `NEG_PUSH` | `-1.40°` | `0.90 s` | `x<=+25 mm` 或 `v<=-80 mm/s` |
...
- 负端完成：`|x+50|<=9 mm`、`|v|<=12 mm/s`，持续 `0.08 s`；
- 总时间限制：`5.00 s`。
...
```text
本标定目录直接由 MaixVision 以“打开文件夹/项目”运行，不使用第一问控制安装包。
```
