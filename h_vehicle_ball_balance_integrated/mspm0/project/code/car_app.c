...
static void car_oled_update_status(uint8 motion_enabled)
{
    ...
    sprintf(line, "TASK:%u M:%u",
            (unsigned)car_context.active_task,
            (unsigned)car_context.mission.state);
    car_oled_show_string(2U, line);
    sprintf(line,
            "VIS:%s C:%u",
            car_context.vision.link_alive ? "ON" : "OFF",
            car_context.vision.ball.confidence);
    car_oled_show_string(4U, line);
...
static void car_stop_and_open_menu(void)
{
    ...
    h_mission_stop(&car_context.mission, 1U, car_context.control_tick);
...
static void car_finish_and_hold_status(void)
{
    ...
    car_context.running_requested = 0U;
    h_mission_finish_chassis(&car_context.mission, car_context.control_tick);
    car_context.ab_stop_pending = 0U;
    car_context.ab_stop_ticks = 0U;
    car_reset_speed_control();
    car_context.finish_state = CAR_FINISH_IDLE;
...
static void car_apply_menu_task(car_task_t task)
{
    ...
    if(CAR_TASK_START == task)
    {
        if(!h_mission_start(&car_context.mission,
                            car_menu_get_speed_tier(),
                            car_context.control_tick))
        {
            car_oled_fill(0x00U);
            car_oled_show_string(0U, "WAIT CAM READY");
            car_oled_show_string(2U, "BACK TO CANCEL");
            return;
        }
        car_context.active_task = car_context.mission.task;
        ...
        tier_params = car_menu_get_speed_tier_params(car_menu_get_speed_tier());
    ...
    {
        car_oled_fill(0x00U);
        car_oled_show_string(0U, "TASK 6 RESERVED");
        car_oled_show_string(2U, "ALGORITHM EMPTY");
        system_delay_ms(1500U);
        car_menu_open();
        car_menu_render();
        return;
    ...
    car_reset_speed_control();
    car_menu_close();
    if(!h_mission_prepare(&car_context.mission, task,
                          car_menu_get_speed_tier(), 0,
                          car_context.control_tick))
    {
        car_oled_fill(0x00U);
    ...
    car_oled_fill(0x00U);
    car_oled_show_string(0U, "CAM PREPARING");
    car_oled_show_string(2U, car_menu_task_name(task));
    car_oled_show_string(6U, "BACK TO CANCEL");
}
...
static void car_send_vision_status(
    ...
    float right_target_rpm)
{
    /* Mission heartbeat is emitted by h_mission_update(). */
    (void)motion_enabled;
    (void)left_target_rpm;
    (void)right_target_rpm;
}
    ...
    float right_target_rpm)
{
    ...
    sprintf(
        log,
        "CAR run=%u mission=%u fault=%u line=%02X err=%d "
        "vision=%u ball=%u conf=%u x10=%d v=%d mission=%u "
        "cmd[L=%d R=%d] rpm100[L=%ld R=%ld] yaw=%+.1f "
        "odo=%.1f packets=%lu crc=%lu ovf=%lu\r\n",
        motion_enabled,
        (unsigned)car_context.mission.state,
        (unsigned)car_context.mission.fault_code,
        car_context.line_sensor.mask,
        car_context.line_sensor.error,
        car_context.vision.link_alive,
        car_context.vision.ball_fresh,
        car_context.vision.ball.confidence,
        car_context.vision.ball.position_x10_mm,
        car_context.vision.ball.velocity_mm_s,
        (unsigned)car_context.mission.state,
        (int)left_target_rpm,
...
void car_app_run(void)
{
    ...
    while(1)
    {
        ...
        vision_uart_get_snapshot(
            car_context.control_tick,
            &car_context.vision);
        h_mission_update(&car_context.mission,
                         &car_context.vision,
                         car_menu_get_speed_tier(),
                         car_context.control_tick);
        if(car_context.mission.state != car_context.previous_mission_state)
        {
            car_context.previous_mission_state = car_context.mission.state;
            car_oled_fill(0x00U);
            if(H_MISSION_READY == car_context.mission.state)
            {
                car_oled_show_string(0U, "READY");
                car_oled_show_string(2U, "OK TO START");
                car_oled_show_string(6U, "BACK TO CANCEL");
            }
            else if(H_MISSION_CONFIGURING == car_context.mission.state)
            {
                car_oled_show_string(0U, "CAM PREPARING");
                car_oled_show_string(2U,
                    car_menu_task_name(car_context.mission.task));
                car_oled_show_string(6U, "BACK TO CANCEL");
            }
            else if(H_MISSION_STARTING == car_context.mission.state)
            {
                car_oled_show_string(0U, "STARTING");
                car_oled_show_string(2U, "TIMER RUNNING");
            }
            else if(H_MISSION_RUNNING == car_context.mission.state)
            {
                car_oled_show_string(0U, "RUNNING");
            }
            else if(H_MISSION_FAULT == car_context.mission.state)
            {
                char fault_line[24];
                car_context.running_requested = 0U;
                car_reset_speed_control();
                car_oled_show_string(0U, "MISSION FAULT");
                sprintf(fault_line, "CODE:%u", car_context.mission.fault_code);
                car_oled_show_string(2U, fault_line);
                car_oled_show_string(6U, "BACK TO MENU");
            }
        }
        ...
        if(car_context.running_requested
           && h_mission_car_allowed(&car_context.mission)
        ...
        left_target_rpm = motion_enabled ? line_left_rpm : 0.0f;
        right_target_rpm = motion_enabled ? line_right_rpm : 0.0f;
        if(car_context.pid_test_active)
        {
