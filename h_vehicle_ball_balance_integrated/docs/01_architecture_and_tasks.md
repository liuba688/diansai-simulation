...
- 红外误差使用 0.30 低通，微分使用 0.20 低通并限幅 100；取消 v1.0.5 大偏差 boost。只有进入弯前20 cm里程候选区且同向滤波误差连续30 ms超过40，才用200 ms渐变接入500 mm圆弧轮速前馈和IMU角速度阻尼。
- 丢线前 50 ms 低速确认，随后正向差速搜索，再短时含反转搜索，仍失败即停车。
- 目标速度每 10 ms 加速最多 4 RPM、减速最多 10 RPM。
- A-B 在 110-140 cm 从档位速度平滑降至 55 RPM，140-150 cm 再降至 25 RPM，150 cm 后仅用正向目标斜坡归零。
- RACE LINE 离开起点宽线并行驶 10 cm 后使能终点检测，550-600 cm 平滑降至 60 RPM；至少行驶 500 cm 后第一次重新经过宽基准线即单向软停，不再倒车寻线或调整车身。
- 摄像头数据从不用于赛道识别或底盘转向。
