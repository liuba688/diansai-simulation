...
MODE_READY 重发最多 3 次；旧 task/run 回包丢弃。STARTED 300 ms 超时、相机心跳
500 ms 超时、平衡任务球状态约 300 ms 不新鲜或 X42S 连续 3 次错误都会禁止底盘并停止摆杆。

## 6. Task 3 算法移植

来源：`maix-h-task3-round-trip-bench-v3.4.5`，配置名
`task3_early_biased_release_v6`。关键动作保持一致：
